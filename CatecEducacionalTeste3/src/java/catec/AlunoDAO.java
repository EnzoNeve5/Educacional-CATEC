...
public class AlunoDAO { // Declara a classe pública AlunoDAO (Data Access Object).
    ...
    public boolean salvarAluno(Aluno aluno) { // Método para salvar um aluno no banco de dados.
        String sql = "INSERT INTO alunos (nome, instituicao, email, senha, data_nascimento) VALUES (?, ?, ?, ?, ?)"; // Define a query SQL para inserção.
        try (Connection conn = DBConnection.getConnection(); // Obtém uma conexão com o banco de dados usando try-with-resources.
            ...
            pstmt.setString(2, aluno.getInstituicao()); // Define o primeiro parâmetro (instituicao) da query.
            pstmt.setString(3, aluno.getEmail()); // Define o segundo parâmetro (email) da query.
            pstmt.setString(4, aluno.getSenha()); // Define o terceiro parâmetro (senha) da query.
             
            // Tratamento para data nula (linha ~21)
            if (aluno.getDataNascimento() != null) { // Verifica se a data de nascimento não é nula.
                pstmt.setDate(5, java.sql.Date.valueOf(aluno.getDataNascimento())); // Define o quarto parâmetro (data de nascimento) como um java.sql.Date.
            }
            
            else {
                pstmt.setNull(5, java.sql.Types.DATE); // Define o quarto parâmetro como NULL no banco de dados se a data for nula.
            }
            ...
            e.printStackTrace(); // Imprime o stack trace do erro (para depuração, usar logger em produção).
    ...
    try (Connection conn = DBConnection.getConnection(); // Obtém uma conexão com o banco de dados.
        PreparedStatement pstmt = conn.prepareStatement(sql)) { // Cria um PreparedStatement.
            ...
            if (rs.next()) { // Se houver um resultado (aluno encontrado).
                Aluno aluno = new Aluno(); // Cria uma nova instância de Aluno.
                aluno.setId(rs.getInt("id")); // Define o ID do aluno.
                aluno.setNome(rs.getString("nome")); // Define o nome do aluno.
                aluno.setInstituicao(rs.getString("instituicao")); // Define a instituição do aluno.
                aluno.setEmail(rs.getString("email")); // Define o e-mail do aluno.
                aluno.setSenha(rs.getString("senha")); // Define a senha do aluno.
                ...
                aluno.setDataNascimento(dataNascimento); // Define a data de nascimento do aluno.
                
                return aluno; // Retorna o objeto Aluno com os dados encontrados.
            }
        }
