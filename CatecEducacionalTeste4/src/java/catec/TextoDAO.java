...
public class TextoDAO {
    ...
    public List<Texto> getAllTextosComAutores() throws SQLException {
        String sql = "SELECT t.id, t.conteudo, t.author_id, t.author_type, " +
                     "COALESCE(p.nome, a.nome) AS author_nome, " +
                     "COALESCE(p.instituicao, a.instituicao) AS author_instituicao, " +
                     "COALESCE(p.email, a.email) AS author_email, " +
                     "COALESCE(p.data_nascimento, a.data_nascimento) AS author_data_nascimento, " +
                     "p.area_atuacao AS author_area_atuacao " +
                     "FROM textos t " +
        ...
        try {
            ...
            while (rs.next()) {
                ...
                texto.setAuthorInstituicao(rs.getString("author_instituicao"));
                texto.setAuthorEmail(rs.getString("author_email"));
                java.sql.Date dataNascimento = rs.getDate("author_data_nascimento");
                texto.setAuthorDataNascimento(dataNascimento != null ? dataNascimento.toLocalDate().toString() : null);
                texto.setAuthorAreaAtuacao(rs.getString("author_area_atuacao"));
                textos.add(texto);
            }
        }
