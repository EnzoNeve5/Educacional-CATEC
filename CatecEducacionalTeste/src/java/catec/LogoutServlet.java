...
public class LogoutServlet extends HttpServlet { // Declara a classe LogoutServlet que estende HttpServlet.
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException { // Sobrescreve o método doGet para lidar com requisições GET.
        HttpSession session = request.getSession(false); // Não cria uma nova sessão se não existir // Obtém a sessão atual sem criar uma nova se não existir.
        if (session != null) { // Verifica se existe uma sessão ativa.
            session.invalidate(); // Invalida a sessão // Invalida a sessão, removendo todos os atributos e encerrando-a.
        }
        response.sendRedirect("index.html"); // Redireciona para a página inicial // Redireciona o usuário para a página inicial (index.html).
    }
